...
namespace VisualDebugger
{
	...
	public:
		...
		void Editer(int SID, int L)
		{
			//make score a string
			std::string Score = std::to_string(L);
			//create a full string 
			string ScoreTxt = "Score:   " + Score;
			//add to screen where availble 
			for (unsigned int i = 0; i < screens.size(); i++)
			{
				//if the screen id maches that of the scoretext
				if (screens[i]->id == SID)
				{
					//
					screens[i]->Editer(ScoreTxt);
					return;
				}
			}

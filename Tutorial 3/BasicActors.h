...
namespace PhysicsEngine
{
	...
	public:
		...
		// - dimensions: 1m
		// - denisty: 1kg/m^3
		Sphere(const PxTransform& pose=PxTransform(PxIdentity), PxReal radius=.3f, PxReal density=2.f) 
			: DynamicActor(pose)
		{ 
...
//		PxMaterial* default_material = physics->createMaterial(.0f, .2f, .0f);
	...
	public:
		...
		// - denisty: 1kg/m^3
		Box(const PxTransform& pose=PxTransform(PxIdentity), PxVec3 dimensions=PxVec3(.5f,.5f,.5f), PxReal density=.7f) 
			: DynamicActor(pose)
		{ 
	...
	public:
		...
		// - dimensions: 1m x 1m x 1m
		// - denisty: 1kg/m^3
		CompoundShape(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.3f, .3f, .3f), PxReal density = .5f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(PxVec3(.6f, 14.f, 1.f)), density); //left
			CreateShape(PxBoxGeometry(PxVec3(.6f, 14.f, 1.f)), density); //right
			CreateShape(PxBoxGeometry(PxVec3(11.f, .6f, 1.f)), density); //top
			CreateShape(PxBoxGeometry(PxVec3(11.f, .6f, 1.f)), density); //bottom
			CreateShape(PxBoxGeometry(PxVec3(11.f, 15.f, .5f)), density); //left bottom
	...
	{
	public:
		Capsule(const PxTransform& pose=PxTransform(PxIdentity), PxVec2 dimensions=PxVec2(.5f,.5f), PxReal density=.5f) 
			: DynamicActor(pose)
		{
	...
	public:
		...
		void DriveVelocity(PxReal value)
		{
			...
			if (actor_0)
			{
				...
				if (actor_0->getRigidDynamicFlags() != PxRigidDynamicFlag::eKINEMATIC)
					actor_0->wakeUp();
			}
			if (actor_1)
			{
		...
		void GearRatio(PxReal Value)
		{
			...
			((PxRevoluteJoint*)joint)->setDriveGearRatio(Value);
		}
		PxReal GearRatio()
		{

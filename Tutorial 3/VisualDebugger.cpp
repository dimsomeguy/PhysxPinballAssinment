...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		///Init renderer
		Renderer::BackgroundColor(PxVec3(113.f/255.f,45.f/255.f,13.f/255.f));
		Renderer::SetRenderDetail(40);
		Renderer::InitWindow(window_name, width, height);
		Renderer::Init();

		camera = new Camera(PxVec3(17.0f, 17.0f, 30.0f), PxVec3(0.f,-.1f,-1.f), 5.f);
	...
	void RenderScene()
	{
		...
		//get the score on each frame
		int score = scene->GetScore();
		
		// add score to screen 
		hud.Editer(HELP, score);
